...
public class ConcurrentQuicksort<T extends Comparable<T>> implements Runnable {
	...
	private ConcurrentQuicksort(SimpleCollection<T> col, int left, int right, int minArraySize, IncrementLatch threadCountDown){
		this.col = col;
		this.left = left;
		this.right = right;
	...
	private void quicksort(int left, int right, boolean initialCall){
		...
		if((mid - 1) - left >= minArraySize){
			...
			THREADPOOL.submit(new ConcurrentQuicksort<T>(col, left, mid - 1, minArraySize, threadCountDown));
